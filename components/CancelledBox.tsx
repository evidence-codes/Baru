...
import GreyBox from "@/assets/images/svg/grey-box.svg";
...
export default function CancelledBox({
  title,
  id,
  progress,
  locationFrom,
  ...
  return (
    <TouchableOpacity onPress={onPress} activeOpacity={0.8}>
      <VStack className="flex-row items-center justify-between rounded-[24px] bg-[#AEAEAE] py-4 my-2 relative overflow-hidden">
        <VStack className="w-1/2 px-4">
          <VStack className="w-1/2 mb-2">
            <VStack className="bg-[#D3D3D3] rounded-full px-2 py-2 items-center justify-center">
        ...
        <VStack className="absolute bottom-0 right-0 rounded-br-[24px] overflow-hidden">
          <GreyBox className="w-full h-full" />
        </VStack>
      </VStack>
    </TouchableOpacity>
  );
