...
import GreyBox from "@/assets/images/svg/big-grey-box.svg";
import Message from "@/assets/images/svg/message.svg";
import Call from "@/assets/images/svg/call.svg";
import { useLocalSearchParams } from "expo-router";

interface CancelledDetailsProps {
  id: string;
  progress: number;
  from: string;
  to: string;
...
export default function CancelledDetailsScreen() {
  const params = useLocalSearchParams() as Partial<CancelledDetailsProps>;
  const {
    id,
    progress,
    from,
  ...
  return (
    <SafeAreaView className="flex-1 bg-[#AEAEAE] px-4">
      <StatusBar style="dark" backgroundColor="#AEAEAE" />
          ...
          <VStack className="">
            <VStack className="flex-row items-center justify-between">
              <VStack className="w-4/6">
                <Text className="font-roboto_bold text-[14px] text-black">
                  Your package was not picked up by the courier
                </Text>
                <Text className="font-roboto_regular text-[12px] text-[#757575]">
